...
use std::{collections::{HashMap, HashSet}, hash::{DefaultHasher, Hash, Hasher}, path::{Path, PathBuf}, sync::{Arc, atomic::{AtomicU64, Ordering}}, time::{Duration, Instant, SystemTime}};
...
struct ActiveSession {
    ...
    audio_switch_waiters: HashMap<u64, (String, tokio::sync::oneshot::Sender<Result<f64, String>>)>,
    seek_tx: Option<tokio::sync::mpsc::Sender<SeekCommand>>,
    timeline: Option<PresentationTimeline>,
}
type DashSessionState = Arc<parking_lot::Mutex<ActiveSession>>;
static ACTIVE: Lazy<DashSessionState> = Lazy::new(|| Arc::new(parking_lot::Mutex::new(ActiveSession::default())));
...
pub(crate) fn cancel_active() {
    ...
    for (_, (_, completion)) in active.audio_switch_waiters.drain() {
        let _ = completion.send(Err("Native DASH session cancelled".into()));
    }
    active.files = None;
...
pub(crate) async fn prepare(config: NativeDashPlaybackConfig) -> Result<String, String> {
    prepare_in_session(config, ACTIVE.clone()).await
}

/// Owns a DASH packet bridge that is independent from the primary player.
/// Canvas multiview slots keep one of these alive so starting another DASH
/// stream cannot cancel either the primary stream or a neighboring slot.
pub(crate) struct DetachedNativeDashSession {
    state: DashSessionState,
}

impl Drop for DetachedNativeDashSession {
    fn drop(&mut self) {
        cancel_session(&self.state);
    }
}

pub(crate) async fn prepare_detached(
    config: NativeDashPlaybackConfig,
) -> Result<(String, DetachedNativeDashSession), String> {
    let state = Arc::new(parking_lot::Mutex::new(ActiveSession::default()));
    let source = prepare_in_session(config, state.clone()).await?;
    Ok((source, DetachedNativeDashSession { state }))
}

fn cancel_session(state: &DashSessionState) {
...
async fn prepare_in_session(config: NativeDashPlaybackConfig, state: DashSessionState) -> Result<String, String> {
    ...
    let result = build_live(&config.manifest_url, &config.request_headers, config.preferred_subtitle_language.as_deref(), kid, key, &cancellation, state.clone()).await;
    match result {
...
pub(crate) fn next_audio_track_id() -> Option<i64> {
    let active = ACTIVE.lock();
    let catalog = active.catalog.as_ref()?;
    if catalog.audio_tracks.len() < 2 { return None; }
    let current = catalog.audio_tracks.iter().position(|track| {
        track.adaptation_set_id == catalog.selected_audio_adaptation_set_id
    }).unwrap_or(0);
    Some(catalog.audio_tracks[(current + 1) % catalog.audio_tracks.len()].mpv_track_id)
}
...
pub(crate) async fn prepare_audio_track_selection(mpv_track_id: i64) -> Result<Option<f64>, String> {
    if mpv_track_id <= 0 { return Ok(None); }
    let (next, adaptation, receive) = {
        let mut active = ACTIVE.lock();
        let Some(tx) = active.switch_tx.as_ref().cloned() else { return Ok(None) };
        let Some(catalog) = active.catalog.as_ref() else { return Ok(None) };
        let Some(track) = catalog.audio_tracks.iter().find(|track| track.mpv_track_id == mpv_track_id) else {
            return Err(format!("Unknown native DASH audio track: {mpv_track_id}"));
        };
        let adaptation = track.adaptation_set_id.clone();
        if catalog.selected_audio_adaptation_set_id == adaptation {
            return Ok(None);
        }
        let next = tx.borrow().generation.saturating_add(1);
        for (_, (_, completion)) in active.audio_switch_waiters.drain() {
            let _ = completion.send(Err("DASH audio switch superseded by newer request".into()));
        }
        let (completion, receive) = tokio::sync::oneshot::channel();
        active.audio_switch_waiters.insert(next, (adaptation.clone(), completion));
        tx.send_modify(|selection| {
            selection.generation = next;
            selection.audio_track = adaptation.clone();
        });
        (next, adaptation, receive)
    };
    log::info!("DASH audio switch requested: track={} adaptation={} generation={}",mpv_track_id,adaptation,next);
    match tokio::time::timeout(Duration::from_secs(20), receive).await {
        Ok(Ok(result)) => result.map(Some),
        Ok(Err(_)) => Err("Native DASH audio switch session ended".into()),
        Err(_) => {
            let mut active = ACTIVE.lock();
            active.audio_switch_waiters.remove(&next);
            if active.catalog.as_ref().is_some_and(|catalog| catalog.selected_audio_adaptation_set_id == adaptation) {
                return Ok(active.timeline.as_ref().map(|timeline| seconds(timeline.current_time)));
            }
            if let (Some(tx), Some(catalog)) = (active.switch_tx.as_ref().cloned(), active.catalog.as_ref()) {
                let stable_audio = catalog.selected_audio_adaptation_set_id.clone();
                let rollback_generation = tx.borrow().generation.saturating_add(1);
                tx.send_modify(|selection| {
                    selection.generation = rollback_generation;
                    selection.audio_track = stable_audio.clone();
                });
            }
            Err("Native DASH audio switch timed out".into())
        }
    }
}

fn complete_selected_audio_switch(session: &DashSessionState, adaptation: &str, result: Result<f64, String>) {
    let mut active = session.lock();
    let generations = active.audio_switch_waiters.iter()
        .filter_map(|(generation, (target, _))| (target == adaptation).then_some(*generation))
        .collect::<Vec<_>>();
    for generation in generations {
        if let Some((_, completion)) = active.audio_switch_waiters.remove(&generation) {
...
async fn build_live(url:&str,request_headers:&HashMap<String,String>,preferred_subtitle_language:Option<&str>,kid:[u8;16],key:[u8;16],cancel:&CancellationToken,session:DashSessionState)->Result<(TempDir,String,bool,DashTrackCatalog,tokio::sync::watch::Sender<SwitchSelection>,tokio::sync::mpsc::Sender<SeekCommand>,PresentationTimeline),String>{
...
impl LiveRuntime {
    async fn prepare_seek(&mut self, command: SeekCommand, socket: &mut TcpStream, cancel: &CancellationToken) {
        ...
        let result = async {
            ...
            if let Some(active_timeline) = self.session.lock().timeline.as_mut() {
    ...
    fn sync_catalog(&self,pending:Option<String>){
        let mut active=self.session.lock();if let Some(catalog)=active.catalog.as_mut(){catalog.selected_video_representation_id=self.snapshot.video.identity.representation.clone();catalog.video_quality_mode=self.selection.video_quality_mode.clone();catalog.pending_video_representation_id=pending;catalog.abr_statistics=self.abr.statistics();catalog.selected_audio_adaptation_set_id=self.selection.audio_track.clone();}
    }
    ...
    async fn reject_switch(&mut self,generation:u64,cancel:&CancellationToken)->Result<(),String>{let mut rollback=self.stable_selection.clone();rollback.generation=generation;self.apply_selection(rollback,cancel).await?;self.stable_selection=self.selection.clone();self.switch_state=SwitchState::Stable;let mut active=self.session.lock();if let Some(catalog)=active.catalog.as_mut(){catalog.selected_video_representation_id=self.selection.video_representation.clone();catalog.selected_audio_adaptation_set_id=self.selection.audio_track.clone();}Ok(())}
    async fn refresh(&mut self,cancel:&CancellationToken)->Result<(),String>{
        ...
        let period_changed=self.snapshot.video.identity.period!=candidate.video.identity.period;
        ...
        for track in &candidate.audio_tracks{let changed=self.snapshot.audio_tracks.iter().find(|old|old.adaptation_set_id==track.adaptation_set_id).is_some_and(|old|old.selected.identity!=track.selected.identity||old.selected.template.initialization!=track.selected.template.initialization||old.selected.codecs!=track.selected.codecs||old.selected.audio_sampling_rate!=track.selected.audio_sampling_rate);if changed||!self.audio_inits.contains_key(&track.adaptation_set_id){self.audio_inits.insert(track.adaptation_set_id.clone(),fetch_init(&self.client,&track.selected,cancel).await?);self.audio_generations.insert(track.adaptation_set_id.clone(),self.next_audio_generation);self.next_audio_generation+=1;}}
        for rep in &candidate.subtitles{let changed=self.snapshot.subtitles.iter().find(|old|old.identity.adaptation==rep.identity.adaptation).is_none_or(|old|old.identity!=rep.identity||old.template.initialization!=rep.template.initialization||old.codecs!=rep.codecs);if changed||!self.subtitle_inits.contains_key(&rep.identity.adaptation){self.subtitle_inits.insert(rep.identity.adaptation.clone(),fetch_init(&self.client,rep,cancel).await?);}}
        let stats=self.index.merge(&candidate)?;
        if period_changed&&!self.index.records.values().any(|record|record.advertised&&record.descriptor.identity.representation==candidate.video.identity&&record.descriptor.start==self.frontier){if let Some(next)=descriptors(&candidate.video,candidate.dynamic)?.into_iter().filter(|segment|segment.start>=self.frontier).min_by_key(|segment|segment.start){log::info!("DASH Period transition {} -> {} frontier={:.3} -> {:.3}",self.snapshot.video.identity.period,candidate.video.identity.period,seconds(self.frontier),seconds(next.start));self.frontier=next.start;}}
        if stats.added==0{log::info!("native DASH refresh produced no newer media");}log_snapshot(&candidate);log_merge(&candidate,&stats);self.snapshot=candidate;let mut catalog=build_catalog(&self.snapshot);catalog.video_quality_mode=self.selection.video_quality_mode.clone();catalog.abr_statistics=self.abr.statistics();let mut active=self.session.lock();catalog.selected_audio_adaptation_set_id=self.selection.audio_track.clone();active.catalog=Some(catalog);if let Some(old)=active.timeline.as_ref(){let timeline=presentation_timeline(&self.snapshot,self.origin,old.current_time)?;let state=timeline.state();log::info!("DASH DVR: seek_start={:.3} seek_end={:.3} live_edge={:.3} window={:.3}s",state.seek_range_start,state.seek_range_end,state.live_edge,state.window_duration);active.timeline=Some(timeline);}Ok(())
    }
}
...
impl RdpBatch{
    ...
    fn bridge_ttml(&mut self,bases:&[(u32,i128,u64)],seen:&mut HashSet<String>)->Result<(),String>{
        ...
        let mut input_documents=0usize;let mut parsed_cues=0usize;let mut emitted_cues=0usize;let mut duplicate_cues=0usize;let mut empty_cues=0usize;let mut emitted_range:Option<(i64,i64)>=None;
        let mut output=Vec::new();let mut pos=0;while pos<self.records.len(){let size=u32_at(&self.records,pos+48)? as usize;let end=pos+52+size;let track=u32_at(&self.records,pos+4)?;if !subtitle_ids.contains(&track){output.extend_from_slice(&self.records[pos..end]);pos=end;continue}let Some((_,base_media,timescale))=bases.iter().find(|b|b.0==track)else{output.extend_from_slice(&self.records[pos..end]);pos=end;continue};input_documents+=1;let tb_num=i32_at(&self.records,pos+36)?;let tb_den=i32_at(&self.records,pos+40)?;let packet_pts=i64_at(&self.records,pos+12)?;let packet_start=ExactTime::new((packet_pts as i128).checked_mul(tb_num as i128).ok_or("TTML time overflow")?,tb_den as u64).map_err(|_|"TTML time overflow")?;let media_base=ExactTime::new(*base_media,*timescale).map_err(|_|"DASH timestamp overflow")?;
            for parsed in crate::ttml::parse_document(&self.records[pos+52..end])?{parsed_cues+=1;let(start_time,end_time,payload)=match parsed{crate::ttml::ParsedCue::Text(cue)=>{if !cue.unsupported.is_empty(){log::warn!("[native-dash] unsupported TTML features track={} features={}",track,cue.unsupported.join(","))}(cue.start,cue.end,crate::ttml::ass_payload(&cue).into_bytes())},crate::ttml::ParsedCue::Bitmap(cue)=>(cue.start,cue.end,crate::ttml::bitmap_payload(&cue)?)};let original_start=start_time.checked_sub(media_base).map_err(|_|"TTML time overflow")?;let end_time=end_time.checked_sub(media_base).map_err(|_|"TTML time overflow")?;let mut hasher=DefaultHasher::new();payload.hash(&mut hasher);let key=format!("{track}:{}/{}:{}/{}:{:016x}",original_start.numerator(),original_start.denominator(),end_time.numerator(),end_time.denominator(),hasher.finish());if !seen.insert(key){duplicate_cues+=1;continue}let start=original_start.max(packet_start);let pts=i64::try_from(start.rescale(1000,crate::dash_timeline::Rounding::NearestTiesAway).map_err(|_|"TTML time overflow")?).map_err(|_|"TTML time overflow")?;let duration=i64::try_from(end_time.checked_sub(start).map_err(|_|"TTML time overflow")?.rescale(1000,crate::dash_timeline::Rounding::NearestTiesAway).map_err(|_|"TTML time overflow")?).map_err(|_|"TTML time overflow")?;if duration>0{append_packet(&mut output,track,pts,duration,&payload);emitted_cues+=1;let end=pts.saturating_add(duration);emitted_range=Some(emitted_range.map_or((pts,end),|(first,last)|(first.min(pts),last.max(end))))}else{empty_cues+=1}}pos=end}
        if !bases.is_empty(){let range=emitted_range.map(|(first,last)|format!("{first}..{last}ms")).unwrap_or_else(||"none".into());log::info!("[native-dash/subtitle] TTML bridge documents={} parsed_cues={} emitted_cues={} duplicate_cues={} nonpositive_cues={} pts_range={}",input_documents,parsed_cues,emitted_cues,duplicate_cues,empty_cues,range)}
        self.records=output;Ok(())
    }
}
...
mod tests {
    ...
    #[test]
    fn dynamic_multi_period_snapshot_uses_latest_period_and_accepts_cbcs() {
        let period=|id:&str,start:u32,kid:&str|format!(r#"<Period id="{id}" start="PT{start}S"><AdaptationSet id="v" contentType="video" mimeType="video/mp4"><ContentProtection schemeIdUri="urn:mpeg:dash:mp4protection:2011" value="cbcs" cenc:default_KID="{kid}"/><SegmentTemplate timescale="1" initialization="{id}-v-init" media="{id}-v-$Time$"><SegmentTimeline><S t="0" d="5" r="3"/></SegmentTimeline></SegmentTemplate><Representation id="v1" bandwidth="100"/></AdaptationSet><AdaptationSet id="a" contentType="audio" mimeType="audio/mp4"><ContentProtection schemeIdUri="urn:mpeg:dash:mp4protection:2011" value="cbcs" cenc:default_KID="{kid}"/><SegmentTemplate timescale="1" initialization="{id}-a-init" media="{id}-a-$Time$"><SegmentTimeline><S t="0" d="5" r="3"/></SegmentTimeline></SegmentTemplate><Representation id="a1" bandwidth="50"/></AdaptationSet></Period>"#);
        let data=format!(r#"<MPD type="dynamic" xmlns:cenc="urn:mpeg:cenc:2013">{}{}</MPD>"#,period("program",0,"5bc51e1a-fdf0-3dc0-aa9d-3a412a6dcecf"),period("ad",20,"5bc51e1a-fdf0-3dc0-aa9d-3a412a6dcecf"));
        let snapshot=snap(data.as_bytes(),1);
        assert_eq!(snapshot.periods,vec!["program","ad"]);
        assert_eq!(snapshot.video.identity.period,"ad");
        assert_eq!(snapshot.video.index.get(1).unwrap().unwrap().presentation_start,ExactTime::new(20,1).unwrap());
        assert_eq!(snapshot.video.template.initialization.as_deref(),Some("ad-v-init"));
    }
