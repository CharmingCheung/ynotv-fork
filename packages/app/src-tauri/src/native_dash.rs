...
impl LiveRuntime {
    async fn make_batch(&mut self,video:&[SegmentDescriptor],audio:&[SegmentDescriptor],subtitles:&[Vec<SegmentDescriptor>],cancel:&CancellationToken)->Result<RdpBatch,String>{
        ...
        for segment in video.iter().chain(audio).chain(subtitles.iter().flatten()){if self.index.fetched.contains(&segment.identity){return Err("Duplicate DASH segment fetch prevented".into())}self.index.mark(&segment.identity,SegmentState::Fetched);log::info!("DASH segment fetched identity={}",segment.identity.media_time);}
...
impl RdpBatch{
    ...
    fn bridge_ttml(&mut self,bases:&[(u32,i128,u64)],seen:&mut HashSet<String>)->Result<(),String>{
        ...
        for config in &mut self.configs{if u32_at(config,8)?!=3{continue}let id=u32_at(config,0)?;if !bases.iter().any(|b|b.0==id){continue}let mut replacement=config[..68].to_vec();replacement.extend((crate::ttml::ASS_HEADER.len() as u32).to_le_bytes());replacement.extend(crate::ttml::ASS_HEADER.as_bytes());*config=replacement}
        let mut output=Vec::new();let mut pos=0;while pos<self.records.len(){let size=u32_at(&self.records,pos+48)? as usize;let end=pos+52+size;let track=u32_at(&self.records,pos+4)?;if !subtitle_ids.contains(&track){output.extend_from_slice(&self.records[pos..end]);pos=end;continue}let Some((_,base_media,timescale))=bases.iter().find(|b|b.0==track)else{output.extend_from_slice(&self.records[pos..end]);pos=end;continue};let tb_num=i32_at(&self.records,pos+36)?;let tb_den=i32_at(&self.records,pos+40)?;let packet_pts=i64_at(&self.records,pos+12)?;let packet_start=ExactTime::new((packet_pts as i128).checked_mul(tb_num as i128).ok_or("TTML time overflow")?,tb_den as u64).map_err(|_|"TTML time overflow")?;let cues=crate::ttml::parse_document(&self.records[pos+52..end])?;for cue in cues{let crate::ttml::ParsedCue::Text(cue)=cue else{return Err("Unsupported IMSC image profile: bitmap compositor adapter unavailable".into())};if !cue.unsupported.is_empty(){log::warn!("[native-dash] unsupported TTML features track={} features={}",track,cue.unsupported.join(","))}let media_base=ExactTime::new(*base_media,*timescale).map_err(|_|"DASH timestamp overflow")?;let original_start=cue.start.checked_sub(media_base).map_err(|_|"TTML time overflow")?;let end_time=cue.end.checked_sub(media_base).map_err(|_|"TTML time overflow")?;let key=format!("{track}:{}/{}:{}/{}:{}",original_start.numerator(),original_start.denominator(),end_time.numerator(),end_time.denominator(),crate::ttml::ass_payload(&cue));if !seen.insert(key){continue}let start=original_start.max(packet_start);let pts=i64::try_from(start.rescale(1000,crate::dash_timeline::Rounding::NearestTiesAway).map_err(|_|"TTML time overflow")?).map_err(|_|"TTML time overflow")?;let duration=i64::try_from(end_time.checked_sub(start).map_err(|_|"TTML time overflow")?.rescale(1000,crate::dash_timeline::Rounding::NearestTiesAway).map_err(|_|"TTML time overflow")?).map_err(|_|"TTML time overflow")?;if duration<=0{continue}let payload=crate::ttml::ass_payload(&cue).into_bytes();output.extend(0x31544b50u32.to_le_bytes());output.extend(track.to_le_bytes());output.extend(1u32.to_le_bytes());output.extend(pts.to_le_bytes());output.extend(pts.to_le_bytes());output.extend(duration.to_le_bytes());output.extend(1i32.to_le_bytes());output.extend(1000i32.to_le_bytes());output.extend(0u32.to_le_bytes());output.extend((payload.len() as u32).to_le_bytes());output.extend(payload)}pos=end}self.records=output;Ok(())
    }
}
...
mod tests {
    ...
    fn ttml_bridge_clips_leading_time_and_deduplicates_adjacent_documents() {
        fn config()->Vec<u8>{let mut c=Vec::new();for v in [3u32,1,3]{c.extend(v.to_le_bytes())}for v in [1i32,1000,0,0,0,0]{c.extend(v.to_le_bytes())}let mut codec=[0u8;32];codec[..4].copy_from_slice(b"ttml");c.extend(codec);c.extend(0u32.to_le_bytes());c}
        fn record(xml:&[u8])->Vec<u8>{let mut r=Vec::new();r.extend(0x31544b50u32.to_le_bytes());r.extend(3u32.to_le_bytes());r.extend(1u32.to_le_bytes());r.extend(0i64.to_le_bytes());r.extend(0i64.to_le_bytes());r.extend(8000i64.to_le_bytes());r.extend(1i32.to_le_bytes());r.extend(1000i32.to_le_bytes());r.extend(0u32.to_le_bytes());r.extend((xml.len()as u32).to_le_bytes());r.extend(xml);r}
        ...
        assert_eq!(&batch.configs[0][36..40],b"ttml");assert!(batch.configs[0][72..].starts_with(b"[Script Info]"));let mut pos=0;let mut pts=Vec::new();while pos<batch.records.len(){pts.push(i64_at(&batch.records,pos+12).unwrap());pos+=52+u32_at(&batch.records,pos+48).unwrap()as usize}assert_eq!(pts,vec![0,7500]);assert_eq!(seen.len(),2);
    }
}
