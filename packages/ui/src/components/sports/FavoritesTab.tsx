...
interface SortableFavoriteCardProps {
  ...
  onTogglePin: (teamId: string, leagueId?: string) => void;
  onRemove: (teamId: string, leagueId?: string) => void;
...
const SortableFavoriteCard = memo(
  ...
  const sortableId = getFavKey(team);
  const {
    attributes,
    listeners,
    setNodeRef,
    ...
    >
      ...
      <div className="favorite-card-controls" onPointerDown={(e) => e.stopPropagation()}>
        <button
          ...
          onClick={(e) => {
            e.stopPropagation();
            onTogglePin(team.id, team.leagueId);
        ...
        <button
          ...
          onClick={(e) => {
            e.stopPropagation();
            onRemove(team.id, team.leagueId);
...
export function FavoritesTab({ onSearchChannels, onPlayChannel, onSetTab }: FavoritesTabProps) {
  ...
  const todayFavoriteGames = useMemo(() => {
    ...
    favorites.forEach((team) => {
      const { liveEvent, nextEvent } = teamGameMap[getFavKey(team)] || {};
  ...
  const sortedFavorites = useMemo(() => {
    return [...favorites].sort((a, b) => {
      ...
      const aLive = teamGameMap[getFavKey(a)]?.liveEvent ? 1 : 0;
      const bLive = teamGameMap[getFavKey(b)]?.liveEvent ? 1 : 0;
  ...
  return (
    <div className="sports-tab-content favorites-tab-container">
      ...
      <section className="sports-section favorites-grid-section">
        ...
        >
          <SortableContext
            items={sortedFavorites.map((t) => getFavKey(t))}
