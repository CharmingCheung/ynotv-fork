 ...
 * of waiting out the cache TTL. The marker is versioned so that only ever happens once.
 */
...
export interface StalkerCategoryCacheMarker {
  ...
  /** True when the marker was written by a build that could cache a truncated category. */
  legacy: boolean;
}

const EMPTY_MARKER: StalkerCategoryCacheMarker = { syncedAt: null, legacy: false };
...
export function readStalkerCategoryCacheMarker(raw: string | null | undefined): StalkerCategoryCacheMarker {
  ...
  if (Number.isFinite(asNumber) && asNumber > 0) {
    return { syncedAt: asNumber, legacy: true };
  }
  ...
  try {
    ...
    return {
      syncedAt: ts,
      legacy: !Number.isFinite(version) || version < STALKER_CATEGORY_CACHE_VERSION,
    };
  } catch {
    return EMPTY_MARKER;
  }
...
export function writeStalkerCategoryCacheMarker(syncedAt: number = Date.now()): string {
  return JSON.stringify({ v: STALKER_CATEGORY_CACHE_VERSION, ts: syncedAt });
}

/**
 * True when a cached category was written by a build that could only fetch its first page.
 * The caller must pass the real row count: one page (or fewer) of items is the only signature
 * those builds could produce for a category that actually holds more.
 */
